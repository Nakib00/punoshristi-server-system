  ...
  return (
    <div className="bg-surface text-on-surface min-h-screen pb-xl">
      <header className="bg-surface sticky top-0 z-50 flex items-center gap-md px-margin-mobile h-16 w-full">
        ...
        <h1 className="font-headline-lg-mobile text-headline-lg-mobile font-bold text-primary">{t('history.title')}</h1>
      </header>

      <main className="px-margin-mobile pt-md space-y-sm">
        {loading && <p className="font-body-md text-body-md text-on-surface-variant">{t('history.loading')}</p>}
        {!loading && activity.length === 0 && (
          <p className="font-body-md text-body-md text-on-surface-variant">{t('history.noActivity')}</p>
        )}
        {activity.map((a) => (
