  ...
  return (
    <div className="bg-primary-container text-white overflow-x-hidden min-h-screen flex flex-col justify-center items-center px-margin-mobile relative">
      ...
      <main className="w-full max-w-md flex flex-col items-center text-center z-10">
        ...
        <h1 className="font-headline-xl text-headline-xl text-white mb-xs">{t('success.pointsEarned', { points: result.earnedPoints })}</h1>
        <p className="font-body-lg text-body-lg text-primary-fixed/90 mb-xl">
          {t('success.bottlesRecycled', { count: result.addedBottles, plural: result.addedBottles === 1 ? '' : 's' })}
        </p>

        <div className="w-full bg-white rounded-xl p-lg shadow-2xl flex flex-col gap-md mb-xl text-on-surface">
          <div className="grid grid-cols-3 gap-sm">
            <div className="flex flex-col items-center">
              <span className="text-on-surface-variant font-label-md text-label-md mb-xs">{t('success.totalPoints')}</span>
            ...
            <div className="flex flex-col items-center border-x border-outline-variant/30">
              <span className="text-on-surface-variant font-label-md text-label-md mb-xs">{t('success.bottlesToday')}</span>
              <span className="text-primary font-headline-lg-mobile text-headline-lg-mobile">{stats?.bottlesToday ?? '—'}</span>
            </div>
            <div className="flex flex-col items-center">
              <span className="text-on-surface-variant font-label-md text-label-md mb-xs">{t('success.currentRank')}</span>
          ...
          <div className="flex flex-col gap-xs text-left">
            <div className="flex justify-between items-center">
              <span className="text-on-surface font-title-md text-title-md">
                {toNext !== null ? t('success.roadToLevel', { level: stats.level.level + 1 }) : t('success.maxLevel')}
              </span>
              {toNext !== null && (
                <span className="text-on-surface-variant font-label-md text-label-md">{t('success.ptsToGo', { pts: toNext })}</span>
              )}
            </div>
